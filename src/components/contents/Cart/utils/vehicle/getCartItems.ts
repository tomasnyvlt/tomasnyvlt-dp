...
const getItems = ({ fetcherData, formValues }: GetCartDataProps<"autosjednavac">): CartItemType[] => {
  const { mtpl, casco, assistance, additionals } = fetcherData || {};
  const items: CartItemType[] = [];

  // POV
  if (mtpl?.selected && mtpl?.premiumTotal > 0 && !!mtpl?.basicLimits?.length) {
    const selectedLimit = mtpl.basicLimits.find((limit) => limit.premium === mtpl.premium);
    ...
    if (pickedId) {
      ...
      if (cascoPackage?.name) {
        items.push({
          ...
          formattedPrice: getFormattedPrice({ price: casco.premiumTotal, withYear: !fetcherData?.shortTerm }) as string,
          removable: false
        });
      }
    }
